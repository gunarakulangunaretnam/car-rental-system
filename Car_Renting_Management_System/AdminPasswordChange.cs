...
namespace Car_Renting_Management_System
{
    public partial class AdminPasswordChange : Form
    {
        ...
        private void AdminPasswordChange_Load(object sender, EventArgs e)
        {
            ...
            xmlDoc.Load(path + "\\CRMS\\adminCredentials.xml");
        ...
        private void bunifuFlatButton5_Click(object sender, EventArgs e)
        {
            ...
            {
                if (monoFlat_TextBox1.Text == pass)
                {
                    if (monoFlat_TextBox2.Text.Length >= 8)
                    {
                        string path = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
                        XmlDocument xmlDoc = new XmlDocument();
                        xmlDoc.Load(path + "\\CRMS\\adminCredentials.xml");
                        xmlDoc.SelectSingleNode("LoginInfo/password").InnerText = MySecurity.Encrypt(monoFlat_TextBox2.Text, key);
                        xmlDoc.Save(path + "\\CRMS\\adminCredentials.xml");

                        string pathforLog = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
                        string txt = "The Admin Password was changed on " + DateTime.Now.ToString("yyyy-MM-dd") + " at " + DateTime.Now.ToLongTimeString();
                        File.AppendAllText(pathforLog + "//CRMS//ActivityLog.txt", txt + Environment.NewLine);

                        SuccessMSGBox s = new SuccessMSGBox("Password has been changed.");
                        s.Show();
                    }
                    else {

                        ErrorMsgBox s = new ErrorMsgBox("The password length must be more than 8 characters.");
                        s.Show();
                ...
                {
                    ErrorMsgBox er = new ErrorMsgBox("The old password is wrong.");
                    er.Show();
            ...
            else
            {
                ...
                if (monoFlat_TextBox1.Text == string.Empty && monoFlat_TextBox2.Text == string.Empty)
                {
                    ...
                    ErrorMsgBox er = new ErrorMsgBox("Enter passwords in both in fields.");
                    er.Show();
                }
                else if (monoFlat_TextBox1.Text == string.Empty)
                {

                    ErrorMsgBox er = new ErrorMsgBox("Enter the old password.");
                    er.Show();

                }
                else if (monoFlat_TextBox2.Text == string.Empty) {

                    ErrorMsgBox er = new ErrorMsgBox("Enter the new password.");
                    er.Show();
                }
        ...
        private void label4_MouseMove(object sender, MouseEventArgs e)
        {
            if (canMove == 1)
            {
                ...
                this.SetDesktopLocation(MousePosition.X -310, MousePosition.Y - 15);
